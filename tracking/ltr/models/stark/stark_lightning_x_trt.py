...
from ltr.models.stark.utils.box_ops import box_xyxy_to_cxcywh
import time
...
class STARKLightningXtrt(nn.Module):
    ...
    def forward_backbone(self, img: torch.Tensor, zx: str, mask: torch.Tensor):
        ...
        if zx == "search":
            ...
            pos_emb_z = self.pos_emb_z0
            pos = pos_emb_z(bs)
        else:
            raise ValueError("zx should be 'template_0' or 'search'.")
        """get the downsampled attention mask"""
        if mask is not None:
            mask_down = F.interpolate(mask[None].float(), size=output_back.shape[-2:])[0]
            """adjust the shape"""
        else:
            mask_down = None
    ...
    def forward_box_head(self, memory, softmax=True):
        ...
        if "CORNER" in self.head_type:
            ...
            # B HW C
            fx = memory[:,-400:,:]
            fx_t = rearrange(fx, 'B (H W) C -> B C H W', H=self.feat_sz_s, W=self.feat_sz_s)
    ...
    def adjust(self, src_feat: torch.Tensor, pos_embed: torch.Tensor, mask: torch.Tensor):
        ...
        feat_vec = rearrange(feat, "B C H W->B (H W) C") # BxHWxC  改成batchfirst 免得各种bug
        #pos_embed_vec = pos_embed.flatten(2).permute(2, 0, 1)  # HWxBxC
        pos_embed_vec = rearrange(pos_embed, "B C H W->B (H W) C")
        #mask_vec = mask.flatten(1)  # BxHW
        if mask is not None:
            mask_vec = rearrange(mask, "B H W->B (H W)")
        else:
            mask_vec = None
