...
class QFNETProcessing(BaseProcessing):
    # Modified from STARK-ST
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            crops, boxes, mask_crops = prutils.jittered_center_crop(data[s + '_images'], jittered_anno,
                                                                              data[s + '_anno'], self.search_area_factor[s],
                                                                              self.output_sz[s], masks=data[s + '_masks'])
            # Apply transforms
            remap = {'template':'train','search':'test'}
            data[s + '_images'], data[s + '_anno'], data[s + '_masks'] = self.transform[remap[s]](
