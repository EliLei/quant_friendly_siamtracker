...
class QFBBActor(BaseActor):
    ...
    def __call__(self, data):
        bb_preds = self.net.get_bb(z=data['template_images'],
                                     x=data['search_images'])

        gtbox = box_xywh_to_xyxy(data['search_anno'])
        gtbox = (gtbox/data['search_images'].shape[-1]).clamp(min=0.0, max=1.0)
        loss_giou,iou = self.objective['giou'](bb_preds, gtbox)
        loss_l1 = self.objective['l1'](bb_preds, gtbox)

        loss = self.loss_weight['giou']*loss_giou + self.loss_weight['l1']*loss_l1
...
class QFClsActor(BaseActor):
    ...
    def __call__(self, data):
        cls_preds = self.net.get_cls(z=data['template_images'],
                                     x=data['search_images'])

        loss_bce = self.objective['bce'](cls_preds, data['label'])
...
import os
class Create_Calibration(BaseActor):
    def __init__(self, net, objective, loss_weight=None, name='qfnet',savemask=False):
        super().__init__(net, objective)
        self.name = name
    ...
    def __call__(self, data):
        ...
        env = env_settings()
        calibration_dir = os.path.join(env.workspace_dir,'calibration',self.name)
        ...
        torch.save(data['template_images'].detach().cpu(), path_z)
        torch.save(data['search_images'].detach().cpu(), path_x)
        if self.savemask:
            torch.save(data['template_masks'].detach().cpu(), path_zmask)
            torch.save(data['search_masks'].detach().cpu(), path_xmask)
            mask1 = data['template_masks'].sum().item()
            mask2 = data['search_masks'].sum().item()
            #print(mask1,mask2)
