...
class MobileNetV2(nn.Module):
    def __init__(self, n_class=1000, width_mult=1.0):
        ...
        self.conv2 = InvertedResidual(int(width_mult*32), int(width_mult*16), 1, 1)
        self.conv3 = nn.Sequential(
            InvertedResidual(int(width_mult*16), int(width_mult*24), 2, 6),
            InvertedResidual(int(width_mult*24), int(width_mult*24), 1, 6)
        )
        self.conv4 = nn.Sequential(
            InvertedResidual(int(width_mult*24), int(width_mult*32), 2, 6),
            InvertedResidual(int(width_mult*32), int(width_mult*32), 1, 6),
            InvertedResidual(int(width_mult*32), int(width_mult*32), 1, 6)
        )
        self.conv5 = nn.Sequential(
            InvertedResidual(int(width_mult*32), int(width_mult*64), 2, 6),
            InvertedResidual(int(width_mult*64), int(width_mult*64), 1, 6),
            InvertedResidual(int(width_mult*64), int(width_mult*64), 1, 6),
            InvertedResidual(int(width_mult*64), int(width_mult*64), 1, 6)
        )
        self.conv6 = nn.Sequential(
            InvertedResidual(int(width_mult*64), int(width_mult*96), 1, 6),
            InvertedResidual(int(width_mult*96), int(width_mult*96), 1, 6),
            InvertedResidual(int(width_mult*96), int(width_mult*96), 1, 6)
        )
        self.conv7 = nn.Sequential(
            InvertedResidual(int(width_mult*96), int(width_mult*160), 2, 6),
            InvertedResidual(int(width_mult*160), int(width_mult*160), 1, 6),
            InvertedResidual(int(width_mult*160), int(width_mult*160), 1, 6)
        )
        self.conv8 = InvertedResidual(int(width_mult*160), int(width_mult*320), 1, 6)
        # Last Conv
        self.conv9 = nn.Sequential(
            nn.Conv2d(int(width_mult*320), int(width_mult*1280), 1, 1, 0, bias=False),
            nn.BatchNorm2d(int(width_mult*1280)),
            nn.ReLU6(inplace=True)
        )
        # Classifier
        self.classifier = nn.Linear(int(width_mult*1280), n_class)
