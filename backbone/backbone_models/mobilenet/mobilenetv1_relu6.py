...
class MobileNetV1Relu6(nn.Module):
    def __init__(self, n_class=1000, width_mult=1.0):
        super(MobileNetV1Relu6, self).__init__()
        ...
        if False:
            # load from mobilenetv1 relu
            state_dict = torch.load('/data/users/leirulin/code/biye/backbone/logs/MobileNetV1_0.31/lightning_logs/version_2/checkpoints/epoch=59-val_loss=1.98-val_acc=0.56.ckpt',map_location='cpu')['state_dict']
            state_dict = {k.lstrip('net.'):v for k,v in state_dict.items()}
